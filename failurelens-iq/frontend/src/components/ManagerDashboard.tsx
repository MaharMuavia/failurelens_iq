...
import { experiments, teamProfiles } from "../data/mockData";
...
function categoryCount(label: string) {
  return experiments.filter((experiment) => experiment.failure_category_label.includes(label)).length;
}

export function ManagerDashboard() {
  const analysis = useAnalysis("EXP-1001");
  const visibleSteps = useSSEStream(analysis.isRunning);
      ...
      <main className="workspace">
        ...
        <section className="status-strip" aria-label="FailureLens summary">
          ...
          <article className="stat-card">
            ...
            <div>
              <strong>{categoryCount("Bias")}</strong>
              <p>bias patterns</p>
            </div>
          </article>
        </section>
