...
class DiagnosticAgent(BaseAgent):
    ...
    async def run(self, ctx: AgentContext) -> AgentContext:
        ...
        evidence = self._evidence(exp, category)
        counter = self._counter_evidence(exp, category)
        ...
        requires_review = score.confidence < threshold or category == FailureCategory.UNKNOWN
        if requires_review:
        ...
        self.complete_trace(ctx, trace, started, steps, evidence or [self.cite_evidence("failure_observation", exp.failure_observation, "available diagnostic text")], score.confidence, [hit.citation for hit in retrieval.hits], counter, score.factors)
        ctx.requires_human_review = ctx.requires_human_review or requires_review
        if requires_review:
            ctx.human_review_reason = f"Diagnosis confidence {score.confidence:.3f} requires human review."
        return ctx
