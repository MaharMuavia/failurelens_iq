...
@pytest.mark.anyio
async def test_iq_status_demo_mode_returns_local_fallback():
...
@pytest.mark.anyio
async def test_iq_status_production_with_mocked_azure_search_returns_live_azure():
    original_mode = settings.APP_MODE
    ...
    payload = response.json()
    assert payload["proof_level"] == "live_azure"
